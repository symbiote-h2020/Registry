    ...
    public void handleDelivery(String consumerTag, Envelope envelope,
        ...
        if (ValidationUtils.validateFields(requestSsp)) {
            ...
            if (sspPersistenceResult.getStatus() == 200) {
                ...
                sspResponse.setBody(sspPersistenceResult.getSmartSpace());
                rabbitManager.sendSspOperationMessage(sspPersistenceResult.getSmartSpace(),
                        RegistryOperationType.CREATION);

            } else {
                log.error("Error occurred during Smart Space saving in db, due to: " +
                        sspPersistenceResult.getMessage());
                sspResponse.setMessage("Error occurred during Smart Space saving in db, due to: " +
                        sspPersistenceResult.getMessage());
